...
class TestPitBoard(AppCase):
    """Three tokens, big, read through a visor at seventy an hour.

    The app cannot show a driver anything — somebody walks to the fence with a
    board. What it can do is compose the line from what is already on the wall,
    and remember what was shown.
    """

    def presets(self, my_row=None, pit_by=None, strategy=None, fatigue=None):
        return self.app.board_presets(my_row or {}, pit_by or {},
                                      strategy or {}, fatigue)

    def test_box_now_is_the_first_thing_on_the_board(self):
        got = self.presets(strategy={"label": "BOX NOW"},
                           pit_by={"seconds": 1800, "reason": "stint limit"})
        self.assertEqual(got[0], "BOX BOX BOX")

    def test_it_counts_the_stint_down_in_minutes(self):
        got = self.presets(pit_by={"seconds": 21 * 60, "reason": "stint limit"})
        self.assertIn("21 MIN", got)

    def test_close_to_the_limit_it_says_box_in(self):
        got = self.presets(pit_by={"seconds": 150, "reason": "stint limit"})
        self.assertIn("BOX IN 2", got)

    def test_never_box_in_zero(self):
        """A board saying BOX IN 0 is a board nobody can act on."""
        got = self.presets(pit_by={"seconds": 20, "reason": "stint limit"})
        self.assertIn("BOX IN 1", got)

    def test_the_gap_is_to_whoever_is_next_on_the_road(self):
        got = self.presets(my_row={"pos": "4", "int_s": 2.46})
        self.assertIn("P4  +2.5", got)

    def test_a_leader_with_nobody_ahead_still_gets_their_position(self):
        got = self.presets(my_row={"pos": "1", "int_s": None})
        self.assertIn("P1", got)

    def test_a_tiring_driver_is_told_to_be_smooth_not_to_push(self):
        got = self.presets(my_row={"pos": "4"},
                           fatigue={"level": "warn", "text": "Dropping off"})
        self.assertIn("SMOOTH", got)
        self.assertNotIn("PUSH", got)

    def test_nothing_to_say_is_an_empty_list_not_a_blank_board(self):
        self.assertEqual(self.presets(), [])

    def test_every_board_fits_on_a_board(self):
        got = self.presets(my_row={"pos": "12", "int_s": 123.456,
                                   "class_pos": 7, "category": "AM"},
                           pit_by={"seconds": 3000, "reason": "stint limit"},
                           strategy={"label": "PUSH"})
        self.assertTrue(got)
        for b in got:
            self.assertLessEqual(len(b), self.app.BOARD_MAX, b)

    def test_the_same_board_is_not_offered_twice(self):
        got = self.presets(my_row={"pos": "1"}, strategy={"label": "BOX NOW"},
                           pit_by={"seconds": 60, "reason": "stint limit"})
        self.assertEqual(len(got), len(set(got)))

    def test_showing_one_records_it(self):
        res = self.client.post("/api/board", json={"text": "BOX BOX BOX"})
        self.assertTrue(res.get_json()["ok"])
        state = self.client.get("/api/state").get_json()
        self.assertEqual(state["board"]["text"], "BOX BOX BOX")
        self.assertEqual(state["board_log"][0]["text"], "BOX BOX BOX")

    def test_the_newest_board_is_the_one_on_top(self):
        for t in ("P4 +2.5", "PUSH", "BOX IN 2"):
            self.client.post("/api/board", json={"text": t})
        log = self.client.get("/api/state").get_json()["board_log"]
        self.assertEqual([r["text"] for r in log[:3]],
                         ["BOX IN 2", "PUSH", "P4 +2.5"])

    def test_the_log_does_not_grow_for_twenty_five_hours(self):
        for i in range(30):
            self.client.post("/api/board", json={"text": "P%d" % i})
        log = self.client.get("/api/state").get_json()["board_log"]
        self.assertEqual(len(log), self.app.BOARD_LOG)

    def test_a_blank_board_is_not_recorded(self):
        self.client.post("/api/board", json={"text": "PUSH"})
        for blank in ("", "   ", None):
            self.client.post("/api/board", json={"text": blank})
        log = self.client.get("/api/state").get_json()["board_log"]
        self.assertEqual(len(log), 1)

    def test_a_long_board_is_cut_to_what_fits(self):
        self.client.post("/api/board",
                         json={"text": "X" * 80})
        board = self.client.get("/api/state").get_json()["board"]
        self.assertEqual(len(board["text"]), self.app.BOARD_MAX)

    def test_whitespace_is_tidied_so_the_board_reads_straight(self):
        self.client.post("/api/board", json={"text": "  BOX   BOX \n BOX  "})
        self.assertEqual(
            self.client.get("/api/state").get_json()["board"]["text"],
            "BOX BOX BOX")
