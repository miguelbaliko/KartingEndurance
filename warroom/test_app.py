...
class TestFindingOurRowByName(AppCase):
    """We do not expect to know our own number on the timing screen, so
    name matching is the only way our row is ever found.

    KIP's own arrive-and-drive sessions carry no team column at all — the
    driver cell is the whole of it, and it changes every time somebody gets
    in — so a session like that loses us entirely.  A real team endurance
    entry list, which is what race day actually is, always has one.
    """
    ...
    def test_a_nameless_feed_loses_us(self):
        self.app.CFG["team_name"] = "TPC CIAO CUORE"
    ...
    def test_a_clipped_name_still_finds_us(self):
        self.app.CFG["team_name"] = "TPC CIAO CUORE"
